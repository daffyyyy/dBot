...
function multi_functions()
{
    $name_days = [
        ...
        "09-09" => "Aniela, Sergiusza, Piotra",
        "09-10" => "Lukasza, Aldony, Sebastiana",
    ...
    if ($cfg['status'] == true)
    {
        if ($cfg['server_name']['status'])
        {
            $name = $dBot->replace_message(null, null, $cfg['server_name']['name']);
            $dBot->query()->serverEdit(['virtualserver_name' => $name]);    
        }
        if ($cfg['random_server_name']['status'])
        {
            $rand_name = $cfg['random_server_name']['names'][rand(0, count($cfg['random_server_name']['names']) - 1)];
            $name = $dBot->replace_message(null, null, $rand_name);
            $dBot->query()->serverEdit(['virtualserver_name' => $name]);    
        }
        if ($cfg['clock']['status'])
        {
            $dBot->query()->channelEdit($cfg['clock']['channel'], ['channel_name' => str_replace('[clock]', date('G:i', time()), $cfg['clock']['name'])]);
        }
        if ($cfg['date']['status'])
        {
            $dBot->query()->channelEdit($cfg['date']['channel'], ['channel_name' => str_replace('[date]', date('d.m.Y', time()), $cfg['date']['name'])]);
        }
        if ($cfg['name_day']['status'])
        {
            $dBot->query()->channelEdit($cfg['name_day']['channel'], ['channel_name' => $name_days[date("m-d")]]);
        }
        if ($cfg['record_online']['status'])
        {
            $cache->get('record_online', $record);
            $info = $dBot->info();
            //print_r($record);
            if ($record < $info['server']['virtualserver_clientsonline'] - $info['server']['virtualserver_queryclientsonline'])
            {
                $record = $info['server']['virtualserver_clientsonline'] - $info['server']['virtualserver_queryclientsonline'];
                $cache->set('record_online', $record);
                $cache->saveCache();
            }

            $dBot->query()->channelEdit($cfg['record_online']['channel'], ['channel_name' => str_replace('[record]', $record, $cfg['record_online']['name'])]);
        }
    }
}